...
corresponding row in the mRNA data, but the variables are different.
...
for(i in 1:ncol(protein)) {
    ...
    protein_adj[names(protein_resid[[i]]), i] <- r
}
...
for(i in 1:ncol(mrna)) {
    ...
    mrna_adj[names(mrna_resid[[i]]),i] <- r
}
...
sim_pve <- corr_betw_matrices(x, y, "all", cores=0)
```
...
table. Also note that `get_problems()` can also be used to look by column
rather than by row.
...
swap, though we can't tell at this point whether it is that protein
...
less-correlated than the optimal one, and so this seems not it is not
mislabeled.
