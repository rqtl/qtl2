...
For the backcross, the new code takes `r median_ratio(probbc)` times
longer; for the intercross, the new code takes 
`r median_ratio(probf2)` times longer.
...
I had some ideas to speed things up. For example, I could
pre-calculate the transition matrices rather than make repeated calls
to the `step` function. However, this may reduce the generality (for
example, that we want to allow separate transition matrices for each
possible value of `cross_info`).

We could just rely on parallel processing to speed things up. And
maybe that's the first thing to try. Other alternatives are to have
separate implementations for different groups of crosses, or to
identify the set of observed `cross_info` values and pre-calculate
emission and transition probabilities for each.

It's also possible that some basic aspects of my code could be
improved, even keeping fixed the overall design. I'm new to C++ and
