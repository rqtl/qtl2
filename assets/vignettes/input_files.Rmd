...
If you've coded the cross direction as in [R/qtl](http://rqtl.org),
...
[R/qtl2geno](https://github.com/rqtl/qtl2geno) package includes data
...
[`grav2`](http://kbroman.org/qtl2/assets/sampledata/grav2/). Here's a
zip file: [`grav2.zip`](http://kbroman.org/qtl2/assets/sampledata/grav2/grav2.zip).
...
`genotype` that look just like those for `riself` and `risib`.
...
cross types is the labels that are attached to the two genotypes:
single-letter codes for haploids and double-letter homozygotes for
doubled haploids.
...
Because the cross information needs two columns, it can be provided
...
the yaml control file shoul contain a line like:
...
recoded before input into R. (The
...
This is similar to HS, but with a specific design that started with
individuals taken from intermediate generations in the
formation of a set of 8-way recombinant inbred lines. See
...
- [`DO_Gatti2014`](https://github.com/rqtl/qtl2data/tree/master/DO_Gatti2014),
  from
...
from any other covariates, the indicates the order
...
integers from 1 to the number of founders.
...
developed at the Wisconsin Crop Innovation Center: A diallel of 6
founders followed by random mating for some number of generations and
then derivation of doubled haploids.
