...
download the following database, which includes both SNP/indel
variants and genes:
...
For querying founder variant information, you can use this
`create_variant_query_func()` as above, using the appropriate path to
the file:

```{r query_variants_build39, eval=FALSE}
query_variants <- create_variant_query_func("~/Data/CCdb/fv.2021.snps.db3")
variants_2_97.5 <- query_variants(2, 97, 98)
```
...
query_variants <- create_variant_query_func("~/Data/CCdb/fv.2021.snps.db3", filter="confidence==255")
variants_2_97.5 <- query_variants(2, 97, 98)
```

The `fv.2021.snps.db3` file also contains [ensembl](https://www.ensembl.org) gene information,
but the field names are different from the defaults, so in your call
to `create_gene_query_func()`, you'll need to specify the field names,
as follows:

```{r create_gene_query_build39, eval=FALSE}
query_genes <- create_gene_query_func("~/Data/CCdb/fv.2021.snps.db3", chr_field="chromosome",
