...
The software is split into multiple packages:
...
scans, [qtl2plot](https://github.com/rqtl/qtl2plot) for data
visualization, and [qtl2db](https://github.com/rqtl/qtl2db) for
connections to SNP and gene databases.
...
probabilities produced by DOQTL to the format needed by qtl2scan.
...
[qtl2db](https://github.com/rqtl/qtl2db),
and
...
genotypes. To plot the effects, use the function `plot_coef()` from
...
estimates. Note that the BLUPs are centered at 0, which the
...
putative QTL position, you can use the function `plot_pxg()` in
[qtl2plot](https://github.com/rqtl/qtl2plot). This takes a vector of
genotypes as produced by the `maxmarg()` function in
[qtl2geno](https://github.com/rqtl/qtl2geno), which picks the most
...
par(mar=c(4.1, 4.1, 0.6, 0.6))
plot_coefCC(coef_c2, DOex$gmap["2"], out, bgcolor="gray95")
legend("bottomleft", col=CCcolors, names(CCcolors), ncol=2, lwd=2, bg="gray95")
```
...
The [qtl2db](https://github.com/rqtl/qtl2db) package provides a
template for how R/qtl2 would connect to SNP and gene databases. It
contains functions `create_variant_query_func()` and
`create_gene_query_func()` that each each return a _function_ for
...
To create a function for querying the CC variants, you load the
[qtl2db](https://github.com/rqtl/qtl2db) package and call
...
region, and `plot_genes()` from the
[qtl2plot](https://github.com/rqtl/qtl2plot) package to plot the gene
locations below the SNP association results. We specific `xlim` for both
panels to ensure that the axes are properly aligned.

```{r plot_snp_asso_with_genes}
par(mfrow=c(2,1), mar=c(4.1, 4.1, 0.6, 0.6))
xlim <- peak_Mbp + c(-1, 1)
plot(out_snps$lod, out_snps$snpinfo, xlim=xlim, drop_hilit=1.5)
genes <- query_genes(2, peak_Mbp - 1, peak_Mbp + 1)
plot_genes(genes, xlim=xlim)
```
