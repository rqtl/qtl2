---
title: "Genotype diagnostics for diversity outbred mice"
output:
    html_document:
        toc: true
        toc_depth: 3
...
Also see the related manuscript, [Broman et al.
(2018)](https://www.biostat.wisc.edu/~kbroman/publications/mpp_diag.pdf).

We will consider, as our example, a subset of the DO data from [Gatti
et al. (2017)](https://doi.org/10.1101/098657).

The genotype data are for a set of 291 mice from DO generations 8 and
11, with genotypes from the
[MegaMUGA array](https://genomics.neogen.com/en/mouse-universal-genotyping-array).
...
[R/qtl2](https://kbroman.org/qtl2) format following the document
["Preparing Diversity Outbred (DO) mouse data for R/qtl2"](https://kbroman.org/qtl2/pages/prep_do_data.html).
...
Let's omit markers without any genotype data and re-code the
genotypes so that the first allele is that which is most common among
the founders.

```{r drop_nulls_and_recode_geno}
...
There are `r add_commas(tot_mar(svenson))` markers
...
data show decreased 1st percentile, but many fit in with the bulk
of the samples.

We're again tempted to omit the `r sum(percent_missing > 5)` samples with >5%
...
generation 8 (in red, with an average of `r round(mean(totxo[svenson$covar$ngen=="8" & percent_missing < 19.97]))`
crossovers) and generation 11 (in blue, with an average of
`r round(mean(totxo[svenson$covar$ngen=="11" & percent_missing < 19.97]))` crossovers).
...
on the error LOD score. The rates differ most for the samples missing
a lot of genotypes.
...
Here are just the samples with estimated error rates < 0.4% (that is,
omitting `r sum(err_direct > 0.5)` samples with error rates > 0.5%).

```{r plot_error_rate_vs_elod0_again}
par(pty="s")
par(mar=c(4.1,4.1,0.6, 0.6))
grayplot(errors_ind_0, err_direct,
         xlab="Percent errors (error LOD > 0)",
...
Panels G and H (`r markers[4]`) are for a case where the
...
meaning that a completely different set of genotypes have positive
...
five markers having absolute difference > 1.5.
...
So here, for individual M305 on chromosome 9, there is a region at
around 72 Mbp where, before data cleaning, the genotype looked to be
AD but after cleaning it looked to be DD. There is a second region
