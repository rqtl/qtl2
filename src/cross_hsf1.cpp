...
enum gen_hsf1 {A=1, H=2, B=3, notA=5, notB=4};

const bool HSF1::check_geno(const int gen, const bool is_observed_value,
...
const double HSF1::init(const int true_gen,
...
const double HSF1::emit(const int obs_gen, const int true_gen, const double error_prob,
...
const double HSF1::step(const int gen_left, const int gen_right, const double rec_frac,
                        const bool is_x_chr, const bool is_female,
                        const IntegerVector& cross_info)
{
    #ifndef RQTL2_NODEBUG
    ...
    const static NumericVector precc_alpha = NumericVector::create(1.0);
...
const bool HSF1::check_is_female_vector(const LogicalVector& is_female, const bool any_x_chr)
{
    bool result = true;
...
const bool HSF1::check_crossinfo(const IntegerMatrix& cross_info, const bool any_x_chr)
{
    bool result = true;
...
const bool HSF1::check_founder_geno_size(const IntegerMatrix& founder_geno, const int n_markers)
{
    bool result=true;
    ...
    if(fg_f != 9) {
        result = false;
        r_message("founder_geno should have 9 founders (the CC founders plus the 9th strain to which the HS individuals were crossed)");
    }
...
const bool HSF1::check_founder_geno_values(const IntegerMatrix& founder_geno)
{
    const int fg_mar = founder_geno.cols();
    const int fg_f   = founder_geno.rows();
...
const bool HSF1::need_founder_geno()
{
    return true;
}

// geno_names from allele names
const std::vector<std::string> HSF1::geno_names(const std::vector<std::string> alleles,
                                                const bool is_x_chr)
{
...
{
    Rcpp::stop("est_map not yet available for HS F1");

    return NA_REAL;
}

// not implemented
const NumericVector HSF1::est_map2(const IntegerMatrix& genotypes,
                                   ...
                                   const bool verbose)
{
    Rcpp::stop("est_map not yet implemented for HS F1s.");
