...
test_that("cbind.scan1() works for scan1_lmm() results", {
    ...
    out1 <- scan1_lmm(probs, grav2$pheno[,1,drop=FALSE], k)
    out2 <- scan1_lmm(probs, grav2$pheno[,2,drop=FALSE], k)
    out12 <- scan1_lmm(probs, grav2$pheno[,1:2], k)
    expect_equal(cbind(out1, out2), out12)

    out1 <- scan1_lmm(probs, grav2$pheno[,1,drop=FALSE], kloco)
    out2 <- scan1_lmm(probs, grav2$pheno[,2,drop=FALSE], kloco)
    out12 <- scan1_lmm(probs, grav2$pheno[,1:2], kloco)
    expect_equal(cbind(out1, out2), out12)
...
test_that("rbind.scan1() works for scan1_lmm() results", {
    ...
    phe <- grav2$pheno[,1,drop=FALSE]
    out1 <- scan1_lmm(probs[,1], phe, k)
    out2 <- scan1_lmm(probs[,2:3], phe, k)
    out3 <- scan1_lmm(probs[,5], phe, k)
    out12 <- scan1_lmm(probs[,1:3], phe, k)
    out12$hsq <- rbind(out1$hsq, out2$hsq) # small adjustment
    out123 <- scan1_lmm(probs[,c(1:3,5)], phe, k)
    out123$hsq <- rbind(out1$hsq, out2$hsq, out3$hsq) # small adjustment
    out2123 <- scan1_lmm(probs[,c(2:3,1,2:3,5)], phe, k)
    ...
    out1 <- scan1_lmm(probs[,1], phe, kloco[1])
    out2 <- scan1_lmm(probs[,2:3], phe, kloco[2:3])
    out3 <- scan1_lmm(probs[,5], phe, kloco[5])
    out12 <- scan1_lmm(probs[,1:3], phe, kloco[1:3])
    out123 <- scan1_lmm(probs[,c(1:3,5)], phe, kloco[c(1:3,5)])
    out2123 <- scan1_lmm(probs[,c(2:3,1,2:3,5)], phe, kloco[c(2:3,1,2:3,5)])
...
test_that("rbind.scan1_lmm() works for scan1_lmm() results with multiple columns", {
