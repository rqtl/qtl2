...
test_that("HSF1 n_gen works", {
    ...
    expect_equal(test_ngen("hsf1", TRUE),  8+8)
...
test_that("HSF1 possible_gen works", {
    ...
    # X male
    expect_equal(test_possible_gen("hsf1", TRUE, FALSE, 20), 8+(1:8))
...
test_that("HSF1 emit works", {
    ...
    for(trueg in c(1,4,7)) { # A, D, G
        for(obsg in 1:5) {
            ...
            expect_equal(test_emit("hsf1", obsg, trueg+8, err, fgen, TRUE, FALSE, 20), expected[obsg])
        }
    }
...
test_that("geno_names works", {
    auto <- LETTERS[1:8]
    X <- c(paste0(LETTERS[1:8], "X"),
           paste0(LETTERS[1:8], "Y"))

    expect_equal(geno_names("hsf1", LETTERS[1:8], FALSE), auto)
    expect_equal(geno_names("hsf1", LETTERS[1:8], TRUE), X)
})
