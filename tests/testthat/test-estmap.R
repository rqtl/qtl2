...
test_that("est_map for backcross autosome matches R/qtl", {
    ...
    chr <- c(3, 4, 17, 19)
    hyper <- hyper[chr,]
    newmap <- est.map(hyper, err=0.002, tol=1e-8) %>% lapply(unclass)
...
test_that("est_map for intercross autosome matches R/qtl", {
    ...
    newmap <- est.map(listeria, err=0.01, tol=1e-8) %>% lapply(unclass)
...
test_that("f2 X chr est_map matches R/qtl", {
    ...
    data(fake.f2)
    fake.f2 <- fake.f2["X",]
    newmap <- est.map(fake.f2, err=0.01, tol=1e-8) %>% lapply(unclass)
...
test_that("bc X chr calc_genoprob matches R/qtl", {
    ...
    newmap <- est.map(cross, err=0.01, tol=1e-8) %>% lapply(unclass)
