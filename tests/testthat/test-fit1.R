...
test_that("fit1 by H-K works", {
    ...
    coef <- lapply(seq_len(length(probs)), function(i) {
        ...
        scan1coef(subset(probs, chr=names(probs)[i]), iron$pheno[,1], addcovar=cov) })
    ...
    out_fit1 <- lapply(seq(along=pmar),
        ...
        fit1(probs[[i]][,,pmar[i]], iron$pheno[,1], addcovar=cov, nullcovar=nullcov) })
