...
addition, a control file (in [YAML](http://www.yaml.org) format),
...
The bulk of the data are in a series of comma-delimited (CSV)
files. Each is in the form of a simple matrix, and for most the first
column is a set of IDs and the first row is a set of variable names.

Missing value codes will be specified in the YAML control file (as
`na.strings`) and will apply across all files, so the missing value
...
(if needed) will generally be columns in the covariate data.
...
So, in general, the cross information will be in a CSV file lines as
...
[YAML](http://www.yaml.org) format) to specify the names of all of the other
files as well as various control parameters such as genotype and sex
encodings and codes for missing values. We use
[YAML](http://www.yaml.org) because it is flexible, human-readable,
and easy to import into R.
...
Any line that begins with a `#` is treated as a comment and
ignored. It's good to include some comments at the top describing the
dataset.

The order of things within the file is not important, but the names
of things are critical.

Much of the information is represented as key-value pairs, as `key:
value`.  For example, the cross type is indicated with a line like
...
The files are expected to be in the same directory as the YAML control
file. They could be placed in a separate directory, with the file name
being the path relative to the location of the control file, but this
is not recommended (or well tested).

The _keys_ for the different files are the following:
...
the corresponding key can be omitted from the YAML control file.
...
The control file should contain a record with `genotypes:` that specifies
...
Here, `covar: sex` indicates that the column name used in the
covariate file is `sex`. If the column name were `Sex`, you would
write `covar: Sex`.

The following two `key: value` pairs indicate the encodings used for
sex, with the keys being the values used in the covariate file and the
values being `female` and `male`. So this indicates that sex was
encoded as `f` for females and `m` for males. If, instead, the sex
covariate had `0` for females and `1` for males, you'd use:
...
So instead of a line with `covar:`, use `file:` followed by the name
of the file (e.g., `file: sex.csv`). You must still provide the sex
encodings, just as before.
...
This is much like the information for sex: the `covar:` line
indicates the name of the column in the covariate data that
corresponds to the cross information. The following two lines indicate
the encodings of the cross information as `key: value` where `key` is
the code used in the cross information column and `value` is the
integer to which it should be converted.

More generally, the cross information would contained in a separate
comma-delimited file.

For simple crosses, in which the cross information is a single column,
we allow it to be encoded differently from what's needed, and the
control file information should look like this:
...
set this up in advance (no translation of encodings will be
...
If a construction like `linemap: value` is used, we look to see if
`value` corresponds to the name of a file; otherwise, we treat it as a
column name in the covariate data. But the use of `covar:` or `file:`
...
semi-colon, or I like the vertical bar (`|`) because it is seldom
...
semicolon doesn't, but it doesn't hurt of you do.
