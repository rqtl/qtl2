...
now be a simple list of matrices of integers. Each component of the
list is a chromosome; the names are the chromosome names. Each
chromosome has a matrix of `n_lines` x `n_markers`.

For many cross types, &ldquo;lines&rdquo; are just the
individuals. For recombinant inbred lines, the genotype data will have
one row per line, which the phenotype data will have one row per
individual, and there will be a separate component (`"linemap"`) for
connecting the individuals to the lines.

The column names are the marker names, and the row names are the line
identifiers.

Describe the genotype encodings. Different treatment of X chromosome
(less effort to convert).

### Chromosome type (`"chrtype"`)
...
### Genotype names (`"genonames"`)

List with `"A"` and `"X"`.

Describe genotype encodings again here.
