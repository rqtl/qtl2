...
A basic principle in the design is to have the data as close as
...
dimensional corresponding to the two alleles.
...
markers. This will have the same form as the genetic map, but with
...
The dimension is `n_phenotypes` &times; `n_pheno_covariates`.
...
For the treatment of the X chromosome, we need access to individuals'
sex. We'll have a logical vector (`"is_female"`) indicating which
...
around?) This will have length `n_individuals`, with the name
...
cross. For an intercross, this is like the `pgm` (for paternal
...
codes for the founders, will be used in various summaries and data
visualizations.
...
arrays; each array corresponds to a chromosome and is of dimension
...
all of the data files, and read that zip file directly (with the same
...
covariate file can be a mixture. The first column in each must be the
individual IDs.
