...
Use the argument `column` to indicate which column to plot. Unlike the
...
plot(out, column=1, col="slateblue", ylim=c(0, ymx*1.02))
plot(out, column=2, col="violetred", add=TRUE)
...
for(i in 1:2) {
    plot(out, column=i, col=color[1], main=colnames(out)[i],
              ylim=c(0, ymx*1.02))
    plot(out_lmm, column=i, col=color[2], add=TRUE)
    plot(out_lmm_loco, column=i, col=color[3], add=TRUE, lty=2)
