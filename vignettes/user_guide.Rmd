...
arranged as individuals &times; genotypes &times; positions. (Well,
actually, the output is a list that includes a bunch of different
components, including a component `"probs"` which this list of 3d arrays, but also
including the genetic map and other details).
...
bidentify a set of LOD peaks that exceed some threshold. It can also
...
```{r find_peaks}
find_peaks(out, threshold=4, drop=1.5)
```
...
find_peaks(out, threshold=4, peakdrop=1.8, drop=1.5)
```
...
```{r bayes_int}
bayes_int(out, lodcolumn=2, chr=9, prob=0.95)
```
...
```{r lod_int_multiple}
lod_int(out, lodcolumn=1, chr=7, peakdrop=1.8, drop=1.5)
```
...
for(i in 1:2) {
    plot(out, lodcolumn=i, col=color[1], main=colnames(iron$pheno)[i],
              ylim=c(0, ymx*1.02))
