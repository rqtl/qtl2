...
ymx <- max(unclass(out)) # overall maximum LOD score
plot_scan1(out, lod=1, col="slateblue", ylim=c(0, ymx*1.02))
plot_scan1(out, lod=2, col="violetred", add=TRUE)
...
for(i in 1:2) {
    plot_scan1(out, lod=i, col=color[1], main=colnames(out)[i],
              ylim=c(0, ymx*1.02))
    plot_scan1(out_lmm, lod=i, col=color[2], add=TRUE)
    plot_scan1(out_lmm_loco, lod=i, col=color[3], add=TRUE, lty=2)
...
plot_coef(c2eff, columns=1:3, col=col)
...
plot_coef(c2effB, columns=2:3, col=col)
for(i in 1:2)
...
col <- c("slateblue", "violetred", "green3")
plot_coef(c2eff_lmm, columns=1:3, col=col, ylab="Phenotype average")
for(i in 1:ncol(c2eff))
...
plot_coef(c2effB_lmm, columns=2:3, col=col)
for(i in 1:2)
