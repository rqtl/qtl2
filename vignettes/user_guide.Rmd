...
Unfortunately, we don't yet have functions for displaying or
summarizing the results of `scan1()`, but you can use the function
`convert2scanone()` to convert the results to the format produced by
`scanone()` in [R/qtl](http://rqtl.org), and then use the relevant
...
plot(out_scanone, lod=1:2, col=c("slateblue", "violetred"), ylab="LOD score", bgrect="gray92",
     bandcol="gray80")
...
To plot the results, we again need to convert them to
the R/qtl `scanone` format.
...
par(mar=c(5.1, 4.1, 1.1, 1.1))
for(i in 1:2) {
  plot(out_scanone, out_lmm_scanone, out_lmm_loco_scanone, lod=i, col=color,
       ylab="LOD score", bgrect="gray92", bandcol="gray80", lty=c(1,1,2),
       main=phenames(iron)[i])
  if(i==2)
...
calculations. So to plot the effects, we could do the following:

```{r plot_effects_liver_c2}
par(mar=c(5.1, 4.1, 1.1, 2.6), las=1)
map <- attr(c2eff, "map")[[1]]
col <- c("slateblue", "violetred", "green3")
plot(map, c2eff[,1], ylim=range(c2eff), xlab="Position (cM)",
     ylab="Phenotype average", col=col[1], type="l")
for(i in 2:ncol(c2eff))
    lines(map, c2eff[,i], col=col[i])
...
par(mar=c(5.1, 4.1, 1.1, 2.6), las=1)
map <- attr(c2effB, "map")[[1]]
ymx <- max(abs(c2effB[,2:3]))
plot(map, c2effB[,2], ylim=c(-ymx, ymx), xlab="Position (cM)",
     ylab="QTL effect", col=col[1], type="l")
lines(map, c2effB[,3], col=col[2])
for(i in 1:2)
...
col <- c("slateblue", "violetred", "green3")
plot(map, c2eff_lmm[,1], ylim=range(c2eff_lmm), xlab="Position (cM)",
     ylab="Phenotype average", col=col[1], type="l")
for(i in 2:ncol(c2eff_lmm))
    lines(map, c2eff_lmm[,i], col=col[i])
for(i in 1:ncol(c2eff))
...
par(mar=c(5.1, 4.1, 1.1, 2.6), las=1)
map <- attr(c2effB_lmm, "map")[[1]]
ymx <- max(abs(c2effB_lmm[,2:3]))
plot(map, c2effB_lmm[,2], ylim=c(-ymx, ymx), xlab="Position (cM)",
     ylab="QTL effect", col=col[1], type="l")
lines(map, c2effB_lmm[,3], col=col[2])
for(i in 1:2)
