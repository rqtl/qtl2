...
package. Unlikely the corresponding function (`calc.genoprob()`) in
[R/qtl](http://rqtl.org), the result is not inserted back into the
...
"leave one chromosome out" (LOCO) method for kinship matrix (scan each
...
```{r scan1_lmm}
out_lmm <- scan1_lmm(pr, iron$pheno, kinship_loco, Xcovar=Xcovar)
```
...
out_lmm <- scan1_lmm(pr, iron$pheno, kinship_loco, Xcovar=Xcovar, cores=4)
```

If we wish to plot the results, we again need to convert them to
...
Here is a plot of the LOD scores, by Haley-Knott and with the linear
mixed model, for the "spleen" phenotype.

```{r plot_lod_scan1_vs_lmm}
color <- c("violetred", "green3")
plot(out_scanone, out_lmm_scanone, lod=2, col=color, ylab="LOD score", bgrect="gray92",
     bandcol="gray80", lty=1:2)
legend("topleft", lwd=2, col=color, c("scan1", "scan1_lmm"), bg="gray92", lty=1:2)
```
