...
We need the physical map of markers, which we take from the `DOex`
object. We can use this map directly, as we'd calculated the allele
...
The output of this function, `snp_pr`, is a list with two components:
\code{"probs"} contains the SNP allele probabilities, and \code{"map"}
contains a map of the SNPs. We'll need each of these, but separately.
...
```{r perform_snp_scan}
out_snps <- scan1(snp_pr$probs, DOex$pheno, k[["2"]], sex)
```
...
par(mar=c(4.1, 4.1, 0.6, 0.6))
plot_snpasso(out_snps, snp_pr$map, drop=1.5)
```
