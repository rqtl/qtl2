...
par(mar=c(5.1, 4.1, 1.1, 1.1))
ymx <- max(rbind(out$lod, out_lmm$lod, out_lmm_loco$lod))
for(i in 1:2) {
    plot(out, lodcolumn=i, col=color[1], main=colnames(out)[i],
              ylim=c(0, ymx*1.02))
    plot(out_lmm, lodcolumn=i, col=color[2], add=TRUE)
    plot(out_lmm_loco, lodcolumn=i, col=color[3], add=TRUE, lty=2)
...
col <- c("slateblue", "violetred", "green3")
plot(c2eff_lmm, columns=1:3, col=col, ylab="Phenotype average")
co <- c2eff$coef
for(i in 1:ncol(co))
    axis(side=4, at=co[nrow(co), i], colnames(co)[i], tick=FALSE, col.axis=col[i])
```
...
par(mar=c(5.1, 4.1, 1.1, 2.6), las=1)
co <- c2effB_lmm$coef[,2:3]
ymx <- max(abs(co))
plot(c2effB_lmm, columns=2:3, col=col)
for(i in 1:2)
    axis(side=4, at=co[nrow(co),i], colnames(co)[i], tick=FALSE, col.axis=col[i])
```
...
```{r DOex_scan1_lmm}
out <- scan1_lmm(apr, DOex$pheno, k, sex)
```
...
We estimate them with `scan1coef_lmm()`. We need to subset the allele
probabilities and the list of kinship matrices.

```{r DOex_effects_c2}
coef_c2 <- scan1coef_lmm(apr[,"2"], DOex$pheno, k[["2"]], sex)
```
...
```{r perform_snp_scan}
out_snps <- scan1_lmm(snppr, DOex$pheno, k[["2"]], sex)
```
